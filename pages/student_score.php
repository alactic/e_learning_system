...
session_start();		
	 ...
	 if(!isset($_SESSION['staff']))
   {
	...
	{
     $user=$_SESSION['staff'];
	 $topic=$_SESSION['course'];
	 $select_ans=$connection->prepare("SELECT * FROM answer WHERE `staff_id`='$user' AND `topic`='$topic'");
	 $select_ans->execute();
	 
	 $select_title=$connection->prepare("SELECT * FROM answer WHERE `staff_id`='$user' AND `topic`='$topic'");
	 $select_title->execute();
	 $fetch_title=$select_title->fetch();
...
<div class="wrapper col3">
  ...
  <h2 style="margin-bottom:-20px">COURSE TITLE: <?php echo "&nbsp;&nbsp;".$fetch_title['topic'];?></h2>
   <table border='1' style='width:500px;'>
    ...
    <?php
	     while($fetch=$select_ans->fetch())
		 {
			 
			 echo "
			 <tr>
			 <td>
			 <h3 style='margin-left:70px'>".$fetch['regno']
			 ."</h3> </td>
			 <td><h3 style='margin-left:70px'>".$fetch['score']."
			 </h3>
			 </td></tr>
			 </br></br>";
		 }
