...
session_start();		
	...
	if($semester==null){
		...
		if($num==0){
		echo "<script> alert('COURSES FOR THIS SEMESTER IS NOT YET OUT')</script>";
		}else{
						$select_home_page=$connection->prepare("SELECT * FROM home_page WHERE `semester`='$semester' AND `year`='$year'");
            $select_home_page->execute();
			$fetch=$select_home_page->fetch();
	
header("Content-Type: application/octet-stream");

$file = "uploads/".$fetch['course']; //.".pdf";
