...
$select=$connection->prepare("SELECT * FROM home_page WHERE `semester`='$semester' AND `year`='$year'");
		 $select->execute();
		 $num=$select->rowCount();
...
if($semester==null||$txtbook==null){
			 ...
			 {
				 $insert=$connection->prepare("INSERT into home_page(`semester`, `course`, `year`) VALUE('$semester', '$txtbook', '$year')");
		         $insert->execute();
			 }else
			 {
				 $update=$connection->prepare("UPDATE `home_page` SET `course`='$txtbook' WHERE `semester`='$semester' AND `year`='$year'");
	             $update->execute();
			 }
...
<div class="wrapper col3">
 ...
 Upload COURSES:<input type="file" name="file"/></br></br>		
